...
{
public:
    // A CoreDispatcher is needed only when running in a XAML app.
    MoveLookController(
        _In_ winrt::Windows::UI::Core::CoreWindow const& window,
        _In_ winrt::Windows::UI::Core::CoreDispatcher const& dispatcher
    );
...
private:
    ...
    void OnGamepadAdded(
        _In_ winrt::Windows::Foundation::IInspectable const& sender,
        _In_ winrt::Windows::Gaming::Input::Gamepad const& gamepad
    );
