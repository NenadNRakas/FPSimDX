...
FPSim::FPSim() :
    ...
    m_currentLevel(0),
    m_levelDuration(0.0),
    m_minBound(0.0,0.0,0.0),
    m_maxBound(0.0,0.0,0.0)
{
}

void FPSim::Initialize(
...
)
{
    ...
    m_savedState.Initialize(ApplicationData::Current().LocalSettings().Values(), L"Game");
    ...
    m_player = std::make_shared<Sphere>(XMFLOAT3(0.0f, -1.3f, 4.0f), 0.2f);
    ...
    world = std::make_shared<GameObject>();
    ...
    for (int a = 0; a < GameConstants::MaxCylinders; a++)
    {
        ...
        {
        case 0:
            cylinder = std::make_shared<Cylinder>(XMFLOAT3(-2.0f, -3.0f, 0.0f), 0.25f, XMFLOAT3(0.0f, 6.0f, 0.0f));
            break;
        case 1:
            cylinder = std::make_shared<Cylinder>(XMFLOAT3(2.0f, -3.0f, 0.0f), 0.25f, XMFLOAT3(0.0f, 6.0f, 0.0f));
            break;
        case 2:
            cylinder = std::make_shared<Cylinder>(XMFLOAT3(0.0f, -3.0f, -2.0f), 0.25f, XMFLOAT3(0.0f, 6.0f, 0.0f));
            break;
        case 3:
            cylinder = std::make_shared<Cylinder>(XMFLOAT3(-1.5f, -3.0f, -4.0f), 0.25f, XMFLOAT3(0.0f, 6.0f, 0.0f));
            break;
        case 4:
            cylinder = std::make_shared<Cylinder>(XMFLOAT3(1.5f, -3.0f, -4.0f), 0.50f, XMFLOAT3(0.0f, 6.0f, 0.0f));
            break;
        }
    ...
    {
        std::shared_ptr<Face> target;
        switch (a)
        {
        case 1:
            target = std::make_shared<Face>(XMFLOAT3(-2.5f, -1.0f, -1.5f), XMFLOAT3(-1.5f, -1.0f, -2.0f), XMFLOAT3(-2.5f, 1.0f, -1.5f));
            break;
        case 2:
            target = std::make_shared<Face>(XMFLOAT3(-1.0f, 1.0f, -3.0f), XMFLOAT3(0.0f, 1.0f, -3.0f), XMFLOAT3(-1.0f, 2.0f, -3.0f));
            break;
        case 3:
            target = std::make_shared<Face>(XMFLOAT3(1.5f, 0.0f, -3.0f), XMFLOAT3(2.5f, 0.0f, -2.0f), XMFLOAT3(1.5f, 2.0f, -3.0f));
            break;
        case 4:
            target = std::make_shared<Face>(XMFLOAT3(-2.5f, -1.0f, -5.5f), XMFLOAT3(-0.5f, -1.0f, -5.5f), XMFLOAT3(-2.5f, 1.0f, -5.5f));
            break;
        case 5:
            target = std::make_shared<Face>(XMFLOAT3(0.5f, -2.0f, -5.0f), XMFLOAT3(1.5f, -2.0f, -5.0f), XMFLOAT3(0.5f, 0.0f, -5.0f));
            break;
        case 6:
            target = std::make_shared<Face>(XMFLOAT3(1.5f, -2.0f, -5.5f), XMFLOAT3(2.5f, -2.0f, -5.0f), XMFLOAT3(1.5f, 0.0f, -5.5f));
            break;
        case 7:
            target = std::make_shared<Face>(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(0.5f, 0.0f, 0.0f), XMFLOAT3(0.0f, 0.5f, 0.0f));
            break;
        case 8:
            target = std::make_shared<Face>(XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(0.5f, 0.0f, 0.0f), XMFLOAT3(0.0f, 0.5f, 0.0f));
            break;
        ...
        //std::vector<byte> ambiantSound = mediaReader.LoadMedia(L"Assets\\Cover.wav");
    ...
    m_level.push_back(std::make_shared<Level1>());
    m_level.push_back(std::make_shared<Level2>());
    m_level.push_back(std::make_shared<Level3>());
    m_level.push_back(std::make_shared<Level4>());
    m_level.push_back(std::make_shared<Level5>());
    m_level.push_back(std::make_shared<Level6>());
    m_levelCount = static_cast<uint32_t>(m_level.size());
...
winrt::Windows::Foundation::IAsyncAction FPSim::LoadLevelAsync()
{
...
void FPSim::PauseGame()
{
    m_timer.Stop();
    SaveState();
}

void FPSim::ContinueGame()
{
    m_timer.Start();
    m_controller->Active(true);
}

GameState FPSim::RunGame()
{
...
void FPSim::OnSuspending()
{
    m_audioController.SuspendAudio();
}

void FPSim::OnResuming()
{
    m_audioController.ResumeAudio();
}

void FPSim::UpdateDynamics()
{
...
#pragma endregion
    ...
    while (timeLeft > 0.0f)
    {
        ...
        for (uint32_t a = 0; a < m_objects.size(); a++)
        {
...
#pragma region inter-ammo collision detection
            if (m_ammoCount > 1)
            {
                for (uint32_t one = 0; one < m_ammoCount; one++)
                {
                    for (uint32_t two = (one + 1); two < m_ammoCount; two++)
                    {
...
#pragma region Apply Gravity and world intersection
            ...
            for (uint32_t i = 0; i < m_ammoCount; i++)
            {
...
void FPSim::SaveState()
{
    // Save basic state of the game.
    m_savedState.SaveBool(L":GameActive", m_gameActive);
    m_savedState.SaveBool(L":LevelActive", m_levelActive);
    m_savedState.SaveInt32(L":LevelCompleted", m_currentLevel);
    m_savedState.SaveInt32(L":TotalShots", m_totalShots);
    m_savedState.SaveInt32(L":TotalHits", m_totalHits);
    m_savedState.SaveSingle(L":BonusRoundTime", m_levelBonusTime);
    m_savedState.SaveXMFLOAT3(L":PlayerPosition", m_player->Position());
    m_savedState.SaveXMFLOAT3(L":PlayerLookDirection", m_controller->LookDirection());

    if (m_levelActive)
    {
        // The game is currently in the middle of a level, so save the extended state of
        // the game.
        m_savedState.SaveSingle(L":LevelDuration", m_levelDuration);
        m_savedState.SaveSingle(L":LevelPlayingTime", m_timer.PlayingTime());

        m_savedState.SaveInt32(L":AmmoCount", m_ammoCount);
        m_savedState.SaveInt32(L":AmmoNext", m_ammoNext);
        ...
        for (uint32_t i = 0; i < m_ammoCount; i++)
        {
            ...
            m_savedState.SaveBool(L":AmmoActive" + string, m_ammo[i]->Active());
            m_savedState.SaveXMFLOAT3(L":AmmoPosition" + string, m_ammo[i]->Position());
            m_savedState.SaveXMFLOAT3(L":AmmoVelocity" + string, m_ammo[i]->Velocity());
        }

        m_savedState.SaveInt32(L":ObjectCount", static_cast<int>(m_objects.size()));
        ...
        for (uint32_t i = 0; i < m_objects.size(); i++)
        {
            ...
            m_savedState.SaveBool(L":ObjectActive" + string, m_objects[i]->Active());
            m_savedState.SaveBool(L":ObjectTarget" + string, m_objects[i]->Target());
            m_savedState.SaveXMFLOAT3(L":ObjectPosition" + string, m_objects[i]->Position());
        }
...
void FPSim::LoadState()
{
    m_gameActive = m_savedState.LoadBool(L":GameActive", m_gameActive);
    m_levelActive = m_savedState.LoadBool(L":LevelActive", m_levelActive);
    ...
    if (m_gameActive)
    {
        ...
        m_totalShots = m_savedState.LoadInt32(L":TotalShots", 0);
        m_totalHits = m_savedState.LoadInt32(L":TotalHits", 0);
        m_currentLevel = m_savedState.LoadInt32(L":LevelCompleted", 0);
        m_levelBonusTime = m_savedState.LoadSingle(L":BonusRoundTime", 0.0f);
        ...
        m_player->Position(
            m_savedState.LoadXMFLOAT3(L":PlayerPosition", XMFLOAT3(0.0f, 0.0f, 0.0f))
        );
        m_camera.Eye(m_player->Position());
        m_camera.LookDirection(
            m_savedState.LoadXMFLOAT3(L":PlayerLookDirection", XMFLOAT3(0.0f, 0.0f, 1.0f))
        );
        m_controller->Pitch(m_camera.Pitch());
        m_controller->Yaw(m_camera.Yaw());
    }
...
void FPSim::SetCurrentLevelToSavedState()
{
    if (m_gameActive)
    {
        if (m_levelActive)
        {
            ...
            m_levelDuration = m_savedState.LoadSingle(L":LevelDuration", 0.0f);

            m_timer.Reset();
            m_timer.PlayingTime(m_savedState.LoadSingle(L":LevelPlayingTime", 0.0f));

            m_ammoCount = m_savedState.LoadInt32(L":AmmoCount", 0);

            m_ammoNext = m_savedState.LoadInt32(L":AmmoNext", 0);
            ...
            storedObjectCount = m_savedState.LoadInt32(L":ObjectCount", 0);
...
void FPSim::SaveHighScore()
{
    m_savedState.SaveInt32(L":HighScore:LevelCompleted", m_topScore.levelCompleted);
    m_savedState.SaveInt32(L":HighScore:TotalShots", m_topScore.totalShots);
    m_savedState.SaveInt32(L":HighScore:TotalHits", m_topScore.totalHits);
}

void FPSim::LoadHighScore()
{
    m_topScore.levelCompleted = m_savedState.LoadInt32(L":HighScore:LevelCompleted", 0);
    m_topScore.totalShots = m_savedState.LoadInt32(L":HighScore:TotalShots", 0);
    m_topScore.totalHits = m_savedState.LoadInt32(L":HighScore:TotalHits", 0);
}

void FPSim::InitializeAmmo()
{
    m_ammoCount = 0;
    m_ammoNext = 0;
    for (uint32_t i = 0; i < GameConstants::MaxAmmo; i++)
    {
